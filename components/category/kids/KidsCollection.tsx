            ...
            <div className="mt-auto">
              {isInCart ? (
                <button
                  ...
                  className="w-full bg-[#1B2559] hover:bg-[#253275] text-white py-1.5 rounded-md font-medium text-[10px] sm:text-xs transition-all duration-300 flex items-center justify-center gap-1 disabled:opacity-50"
                >
                  <>
                    <Trash2 size={14} />
...
export default memo(function KidsCollection({ user }: KidsCollectionProps) {
  ...
  useEffect(() => {
    ...
    user,
    kidsPagination.page,
    kidsPagination.productsPerPage,
    selectedSubcategory,
  ]);
  ...
  return (
    <section className="w-full py-8 sm:py-12 lg:py-16">
      ...
      {/* Pagination */}
      {kidsPagination.totalPages > 1 && !loading && (
        <Pagination
